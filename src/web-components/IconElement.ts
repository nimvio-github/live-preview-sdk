...
const templateHTML = `
  <style>
    ...
    :host {
      ...
      width: var(--app-icon-width, 0px);
      height: var(--app-icon-height, 0px);
    }
...
export class IconElement extends CustomElement {
  ...
  private readonly getTemplateByIconName = (iconName: string | null): HTMLTemplateElement | null => {
    if (!iconName) {
      return null
    }
  ...
  private readonly updateIcon = (iconName: string | null): void => {
    if (!this.shadowRoot) {
      return
    }
